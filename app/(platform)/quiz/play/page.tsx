...
function PlayQuizEngine() {
  ...
  useEffect(() => {
    if (!startedRef.current && state === "idle" && topicsArray.length > 0) {
      ...
      startQuiz(streamParam, topicsArray);
    }
  }, [state, topicsArray, streamParam, startQuiz]);
            ...
            <div className="space-y-2">
                {result.conceptBreakdown.map(concept => (
                    <div key={concept.concept} className="flex justify-between items-center text-xs">
                        <span className="text-foreground truncate pr-2 w-full text-left" title={concept.concept}>{concept.concept}</span>
                        <span className={cn(
                            "font-medium shrink-0",
                             (concept.correct / concept.total) >= 0.8 ? "text-success" : 
                             (concept.correct / concept.total) >= 0.5 ? "text-warning" : "text-destructive"
                        )}>
          ...
          {currentQuestion && (
             <div className="flex items-center gap-2">
                 ...
                 <span className={cn(
                     ...
                     currentQuestion.difficulty === 1 ? "bg-success/10 text-success" :
                     currentQuestion.difficulty === 2 ? "bg-success/20 text-success" :
                     currentQuestion.difficulty === 3 ? "bg-warning/10 text-warning" :
                     currentQuestion.difficulty === 4 ? "bg-destructive/10 text-destructive" :
          ...
          <div className="space-y-2" role="radiogroup" aria-label="Answer options">
            {currentQuestion?.options.map((option) => (
              <QuizOption
                ...
                isCorrect={null} 
                isRevealed={false}
                correctOptionId={""} // Backend grades this
